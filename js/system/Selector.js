...
constructor(domObj){
  if((domObj instanceof(HTMLTextAreaElement)) || (domObj instanceof(HTMLInputElement))){
    ...
    this.domObj = domObj[0]
  }
}
/**
 * Retourne le contenu complet du champ
 */
get fieldValue(){return this.domObj.value}
/**
...
insertAtCaret(v){
  if (this.startOffset || this.startOffset == '0') {
    var startPos = this.startOffset;
    var endPos = this.endOffset;
    this.domObj.value = this.domObj.value.substring(0, startPos)
    + v
    + this.domObj.value.substring(endPos, this.domObj.value.length);
  } else {
      this.domObj.value += v;
  }
}
...
remplace(v){return this.insertAtCaret(v)} // alias
